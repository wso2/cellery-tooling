...
class BalLangServerUtils {
     ...
     */
    public static isLangPluginsAlreadyInstalled() {
        const langServerPluginsJar = path.resolve(Constants.CELLERY_BLANG_SERVER_PLUGINS_DIR,
                                                  Constants.LANG_SERVER_PLUGINS_JAR);
        return fse.existsSync(langServerPluginsJar);
    }
    ...
    public static installLangPlugins(extensionPath: string) {
        ...
        if (langPluginsJarExists) {
            const targetCelleryLangServerLibsJar = path.resolve(Constants.CELLERY_BLANG_SERVER_PLUGINS_DIR,
                                                                Constants.LANG_SERVER_PLUGINS_JAR);
            fse.copyFileSync(langPluginsJar, targetCelleryLangServerLibsJar);
            fse.chmodSync(targetCelleryLangServerLibsJar, 0o666);
        } else {
    ...
    public static uninstallLangPlugins() {
        const langServerPluginsJar = path.resolve(Constants.CELLERY_BLANG_SERVER_PLUGINS_DIR,
                                                  Constants.LANG_SERVER_PLUGINS_JAR);
        const langPluginsJarExists = fse.existsSync(langServerPluginsJar);
        if (langPluginsJarExists) {
            fse.unlinkSync(langServerPluginsJar);
        } else {
